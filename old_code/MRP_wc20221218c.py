...
def MRP_item(arrival, demand, bom):
	...
	h_cost, b_cost = weight_gen_item(T, bom) # weights for holding (product by item array) and backlog (product by item array)
...
# 	tic = time.clock() # for debug
	...
	def ObjRule(model): 
		return sum(h_cost[m]*model.slack[t, m] for m in model.mSet for t in model.tSet) \
				+ sum(b_cost[m]*model.backlog[t, m] \
				for m in model.mSet for t in model.tSet)
