...
def alloc_backlog(df_backlog, demand, bom):
    ...
    for e in backlog_lst:
        ...
        while(backlog_level > 0):
            ...
            if qty > 0 and backlog_level >= qty:
                ...
                ans.append([t, int(e[1]), priorty,qty])                        
                backlog_level -= qty
                
            elif qty > 0 and backlog_level < qty:
                
                ans.append([t, int(e[1]), priorty, backlog_level]) 
                backlog_level = 0
    ...
    if len(df_ans) > 0:
        df_ans.columns = ['time', 'item', 'product', 'backlog_qty']
...
# 	bom = np.array([[1,1,1],[0,0,0],[1,1,0],[1,0,0]])
 	...
 	demand = np.random.randint(0, 10, size=(T, product_size)) # (time by product array) demand @ t 
...
if __name__ == '__main__' :
	...
	case_param = [(10, 10, 10)] #, (10, 10, 6),(15, 6, 70)] # (T, product_size, item_size)
	count = 1
...
# 		tic = time.clock()
		...
		time_end = time.time()
		print(">> MRP_item() in %.5f sec." %(time_end - time_start))
...
# 		tic = time.clock()
		...
		time_end = time.time()
		print(">> MRP() in %.5f sec." %(time_end - time_start))
...
# 		tic = time.clock()
		...
		time_end = time.time()
		print(">> alloc_backlog() in %.5f sec." %(time_end - time_start))
# 		print(">> alloc_backlog() in %.5f sec." %(time.clock()-tic))
...
# 		tic = time.clock()
		...
		time_end = time.time()
		print("\n>> naive_evaluator() in %.5f sec." %(time_end - time_start))
