...
# return a dataframe ["time", "item", "product", "backlog_qty"] with backlog > 0, otherwise return None
def distibution(backlog_qty, demand, bom):
    ...
    for i in a:
        ...
        while(backlog_level > 0):
            ...
            qty = bom[priorty][int(i[1])] * demand[int(i[0])][priorty]
            
            if qty > 0 and backlog_level >= qty:
                            
                dist.append([int(i[0]), int(i[1]), priorty,qty])                        
                backlog_level -= qty
                
            elif qty > 0 and backlog_level < qty:
                
                dist.append([int(i[0]), int(i[1]), priorty,backlog_level]) 
                backlog_level = 0
            
            # go to higher priorty product
            priorty -= 1
            
            if priorty == -1:   priorty = len(demand[0]) - 1
    
    dist.sort()
    dist = pd.DataFrame(dist)
    if len(dist) > 0:
        dist.columns = ['time', 'item', 'product', 'backlog_qty']
...
if __name__ == '__main__' :
	...
	for i in range(round_data):
		...
		test_case = [(random.randint(4,30), random.randint(2,20), random.randint(3,100))]
		...
		for c in test_case:
			...
			time_start = time.time()
			dist = distibution(df_ans, demand, bom)
			time_end = time.time()
			a2 = time_end - time_start
			print(">> Case %d distibution in %.5f sec." %(count, time_end - time_start))
			...
			assert_frame_equal(dist, backlog, check_dtype=False)
